...
def test_run_validate_does_not_apply_e4_against_invalid_duplicate(tmp_path):
    input_dir = tmp_path / "input"
    state_dir = tmp_path / "state"
    output_dir = tmp_path / "output"
    input_dir.mkdir()
    ...
    input_dir.joinpath("applicant_data.csv").write_text(
        "タイムスタンプ,メールアドレス,規約への同意,申請者の学籍番号,申請者の氏名,申請者の学生証写真,共同利用者の有無,共同利用者の学籍番号,共同利用者の氏名,階数希望選択（共同利用者なし）,階数希望選択（共同利用者あり）\n"
        "2026-04-01 00:00:00,test@example.com,利用規約に同意する,1520001,山田 太郎,accept,共同利用者なし (2階・3階のロッカーは使用できません),,,2階,\n"
        "2026-04-01 00:10:00,test@example.com,利用規約に同意する,1520001,山田 太郎,accept,共同利用者なし (2階・3階のロッカーは使用できません),,,4階,\n",
        encoding="utf-8",
    )
    _write_partner_csv(input_dir / "partner_data.csv")
    config_path = tmp_path / "config.yml"
    ...
    assert [row["結果"] for row in rows] == ["E1", "S0"]


def test_run_validate_marks_only_latest_partner_submission_as_active(tmp_path):
    input_dir = tmp_path / "input"
    state_dir = tmp_path / "state"
    output_dir = tmp_path / "output"
    input_dir.mkdir()
    ...
    input_dir.joinpath("applicant_data.csv").write_text(
        "タイムスタンプ,メールアドレス,規約への同意,申請者の学籍番号,申請者の氏名,申請者の学生証写真,共同利用者の有無,共同利用者の学籍番号,共同利用者の氏名,階数希望選択（共同利用者なし）,階数希望選択（共同利用者あり）\n"
        "2026-04-01 00:10:00,test@example.com,利用規約に同意する,1520001,山田 太郎,accept,共同利用者あり,4123456,佐藤 花子,,2階\n",
        encoding="utf-8",
    )
    input_dir.joinpath("partner_data.csv").write_text(
        "タイムスタンプ,メールアドレス,規約への同意,共同利用者の学籍番号,共同利用者の氏名,共同利用者の学生証写真\n"
        "2026-04-01 00:05:00,partner1@example.com,利用規約に同意する,4123456,佐藤 花子,accept\n"
        "2026-04-01 00:08:00,partner2@example.com,利用規約に同意する,4123456,佐藤 花子,accept\n",
        encoding="utf-8",
    )
    config_path = tmp_path / "config.yml"
    config_path.write_text(
